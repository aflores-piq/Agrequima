...
import { cargarOtrosIngresos, obtenerHistorialCargas } from "../../api/cargas";
...
export function CargaOtroIngresoPage() {
  ...
  function cargarHistorial() {
    obtenerHistorialCargas("OtrosIngresos", 1, 10).then((res) => setHistorial(res.filas));
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setResumen(null);
    try {
      const res = await cargarOtrosIngresos(archivo);
      setResumen(res);
      cargarHistorial();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Title className="text-ink">Carga de Otros Ingresos</Title>
        <Text className="text-ink-muted">
          Sube el archivo de Otros Ingresos del mes (.xlsx). Columnas esperadas: Tipo, Concepto,
          Anio, Mes, Valor. Volver a subir el mismo período (año/mes) reemplaza lo ya cargado, no
          lo duplica.
        </Text>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm text-ink-muted">
              Archivo de Otros Ingresos (.xlsx) <span className="text-danger">*</span>
            </label>
            <input
              type="file"
              accept=".xlsx,.xls"
